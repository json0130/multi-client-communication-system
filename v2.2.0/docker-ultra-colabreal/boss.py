...
def main():
    ...
    # Check and install pyttsx3 if not present
    try:
        print("[Main] Checking and installing text-to-speech dependencies...")
        subprocess.run(["pip3", "install", "--quiet", "pyttsx3"], check=True)
        # For Jetson Nano, we may need espeak
        try:
            subprocess.run(["apt-get", "update", "-qq"], check=True)
            subprocess.run(["apt-get", "install", "-y", "-qq", "espeak"], check=True)
        except:
            print("[Main] Note: Could not install espeak. If text-to-speech doesn't work, try installing it manually.")
    except Exception as e:
        print(f"[Main] Warning: Could not install text-to-speech dependencies: {e}")
        print("[Main] You may need to install them manually: pip3 install pyttsx3 and apt-get install espeak")
