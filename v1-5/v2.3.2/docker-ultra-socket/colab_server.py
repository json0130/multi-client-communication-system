...
!pip install flask flask-socketio python-socketio pyngrok pillow torch torchvision matplotlib opencv-python ipywidgets

# colab_server.py - Enhanced real-time emotion detection with streaming and monitoring
import os
import time
import base64
import cv2
...
# Configuration - Enhanced for real-time processing
...
# ENHANCED: More frequent emotion processing for real-time detection
...
# ENHANCED: Real-time emotion processing
frame_counter = 0
last_emotion_process_time = 0
processing_lock = threading.Lock()

# ENHANCED: Advanced emotion tracking with proper moving average
...
# FIXED: Proper moving average implementation
class EmotionTracker:
    ...
    def get_stable_emotion(self):
        ...
        # Calculate weighted average confidence for this emotion
        emotion_confidences = []
        recent_weights = []
        ...
        if emotion_confidences:
            ...
            # Only update if there's significant change or high confidence
            ...
            if (emotion_changed and weighted_confidence > EMOTION_CHANGE_THRESHOLD) or \
               confidence_diff > EMOTION_UPDATE_THRESHOLD:
                ...
                return most_frequent_emotion, weighted_confidence, True  # Changed

        return self.last_stable_emotion, self.last_stable_confidence, False  # No change
...
def setup_ngrok():
    """Setup ngrok tunnel with optimized configuration"""
    try:
        try:
        ...
        ngrok.set_auth_token("2xiJNXVZWQV9QXfg1G43OxNLUyr_3jfRoWsa7TPUbfrKXNMVD")

        tunnel = ngrok.connect(PORT, "http", bind_tls=True)
        public_url = tunnel.public_url
        print(f" Public URL: {public_url}")

        app.ngrok_tunnel = tunnel
        return public_url
...
def decode_frame_optimized(frame_b64):
    ...
    try:
        if len(frame_b64) > 600000:  # Slightly increased limit for higher quality
        ...
        if frame is not None:
            ...
            if width > 800:  # Increased max width for better quality
                scale = 800 / width
...
def should_process_emotion():
    """ENHANCED: More frequent emotion processing"""
    global last_emotion_process_time, frame_counter

    current_time = time.time()
    frame_counter += 1

    # Process emotion more frequently for real-time detection
...
def process_emotion_detection_realtime(frame):
    ...
    try:
        ...
        faces = face_cascade.detectMultiScale(
            gray,
            scaleFactor=1.15,  # More precise detection
            minNeighbors=4,
            minSize=(40, 40)   # Smaller minimum size for better detection
        )
        ...
        try:
            ...
            # ENHANCED: Add to emotion tracker for proper moving average
            ...
            with emotion_lock:
                if emotion_changed or time.time() - last_emotion_update > 1.0:
                    ...
                    if distribution:
                        dist_str = ", ".join([f"{k}: {v:.0f}%" for k, v in distribution.items()])
...
def rate_limit_check(sid):
    ...
    # Allow max 20 messages per second for real-time processing
...
def handle_stream_frame(data):
    ...
    try:
        ...
        # Add face detection bounding box if available
        if face_cascade is not None:
            ...
            for (x, y, w, h) in faces:
                # Draw bounding box
                cv2.rectangle(frame, (x, y), (x+w, y+h), (66, 133, 244), 2)  # Google blue color

                # Add label background
                label_h = 30
                cv2.rectangle(frame, (x, y-label_h), (x+w, y), (66, 133, 244), -1)

                # Add emotion text on the bounding box
        ...
        # Broadcast emotion data more frequently
...
def handle_chat_message(data):
    ...
    # Broadcast to all clients in the stream_viewers room
...
<body>
    ...
    <script>
        ...
        function updateStats(emotion, confidence) {
            ...
            const statsGrid = document.getElementById('emotionStats');
            if (statsGrid) {
                statsGrid.innerHTML = `
                    <div class="stat-card">
                    ...
                    <div class="stat-card">
                        <div class="stat-value">${Math.round(totalConfidence / messageCount)}%</div>
        ...
        function addChatMessage(data) {
            ...
            message.className = `message ${data.type === 'user' ? 'user-message' : 'bot-message'}`;
...
def root_info():
    ...
    return jsonify({
        "message": "Real-time Emotion-Aware System Server",
        "status": "running",
        "components": {
            "model_loaded": model_loaded,
            "transform_loaded": transform_loaded,
...
def initialize_server():
    ...
    print(" Initializing Real-time Emotion Detection Server...")
    print("="*60)
    ...
    print(f"  Transform loaded: {transform_loaded}")
    print(f"  Face cascade loaded: {face_cascade_loaded}")
    print(f"  OpenAI available: {openai_client is not None}")

    # Setup ngrok with retry
    public_url = None
    for attempt in range(3):
        public_url = setup_ngrok()
        if public_url:
            break
        print(f"Retrying ngrok setup... ({attempt + 1}/3)")
        time.sleep(5)

    return public_url

def main():
    """Main server function with real-time optimization"""
    try:
        public_url = initialize_server()

        if public_url:
            print(f"\n Real-time Server: {public_url}")
            print(f" WebSocket: {public_url}/socket.io/")
            print(f" Live Stream: {public_url}/live_stream")
            print(f"  Monitor Interface: {public_url}/monitor")
            print(f" Real-time Config: {STREAM_FPS}fps stream, {1/EMOTION_PROCESSING_INTERVAL:.0f}fps emotion detection")

        print(f"\n Starting real-time emotion server on port {PORT}...")
        print("\n" + "="*60)
