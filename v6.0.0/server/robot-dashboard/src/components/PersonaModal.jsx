...
const DEFAULT_TAGS = ['[DEFAULT]', '[WAVE]', '[HAPPY]', '[SAD]', '[CONFUSED]',
      ...
      <div className="modal">
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            value={form.robot_role}
            onChange={e => setField('robot_role', e.target.value)}
            rows={4}
          />
        </div>
        ...
        <div className="form-group">
          ...
          <div className="checkbox-group">
            {DEFAULT_TAGS.map(tag => (
              <label
                key={tag}
                className={`checkbox-pill ${form.allowed_tags.includes(tag) ? 'checked' : ''}`}
              >
                <input type="checkbox" onChange={() => toggleTag(tag)} />
                {tag}
              </label>
            ))}
          </div>
        ...
        <div className="form-group">
          ...
          <div className="checkbox-group">
            {CAPABILITIES.map(cap => (
              <label
                key={cap}
                className={`checkbox-pill ${form.capabilities[cap] ? 'checked' : ''}`}
              >
        ...
        <div className="form-group">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <select
              ...
              value={form.voice_config.gender}
            ...
            <select
              className="form-select"
              value={form.voice_config.language}
          ...
          <div className="ocean-grid">
            {Object.entries(OCEAN_LABELS).map(([key, label]) => (
              <div key={key} className="ocean-item">
                <label>
                  {label} <span>{form.personality[key].toFixed(1)}</span>
                </label>
                <input
                  type="range" min="0" max="1" step="0.1"
                  className="ocean-slider"
                  value={form.personality[key]}
                  onChange={e => setField(`personality.${key}`, parseFloat(e.target.value))}
                />
              </div>
            ))}
          </div>
