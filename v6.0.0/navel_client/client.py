...
# ── "Power-on and ready" helpers ──────────────────────────────────────────────
# A robot can't be reached by the central server unless it registers a correct IP.
# Rather than hand-editing ip_address per robot, these let the client detect its own
# LAN IP at startup. Used by ServerConnection.register_with_server() below — every
# robot built on BasicClient inherits this for free.

def looks_like_ipv4(value: Optional[str]) -> bool:
    """True only for a real dotted-quad IPv4 (rejects '', None, placeholders)."""
    if not value:
        return False
    try:
...
def detect_local_ip(server_url: str = "") -> str:
    """
    Find this machine's LAN IP — specifically the local address on the interface
    that routes toward the central server. Uses a UDP socket that only *selects* a
    route (no packet is sent), so it works even with no internet, as long as there
    is a default route or the server is on the same subnet. Falls back to hostname
    resolution, then loopback.
    """
    target = (urlparse(server_url).hostname if server_url else None) or "8.8.8.8"
    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    try:
        s.connect((target, 80))
        ip = s.getsockname()[0]
        if looks_like_ipv4(ip) and not ip.startswith("127."):
            return ip
    except Exception:
        pass
    finally:
...
class ServerConnection:
    ...
    async def _handle_connection(self, websocket):
        ...
        if self._ws is not None and self._ws is not websocket:
            # A connection is already active — this is a duplicate dial (e.g. a stale
            # second registration, or a reconnect race). Adopt the newest socket, but
            # the guarded teardown below stops the older one's close from clobbering it.
            logger.warning(f"[WS Server] Second server connection from {addr} — adopting newest.")
        ...
        try:
            ...
            # Prevents a duplicate/older connection's close from clearing _connected
            # (and nulling _ws) out from under a newer, live connection — which would
            # make send()/ACK fail even though the server is still connected.
            if self._ws is websocket:
                self._ws = None
                self._connected.clear()
                logger.info("[WS Server] Waiting for server to reconnect...")
            else:
                logger.info(f"[WS Server] Old/duplicate connection from {addr} closed; active one retained.")
    ...
    def register_with_server(self, ip_address: str = None) -> bool:
        ...
        "Power-on and ready": if no valid ip_address is configured (e.g. a placeholder
        or it is missing), auto-detect this robot's LAN IP so the server can always
        dial back — no per-robot config editing needed. A real configured IP wins.
        """
        ip = ip_address or self.client_config.get("ip_address")
        if not looks_like_ipv4(ip):
            detected = detect_local_ip(self.server_url)
            logger.info(f"[Registration] ip_address '{ip or 'unset'}' is not a valid IP — "
                        f"auto-detected {detected}")
            ip = detected
            # Persist so startup banners / later reads see the real address.
            self.client_config["ip_address"] = ip
