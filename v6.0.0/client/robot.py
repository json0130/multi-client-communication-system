...
class SimpleConcurrentClient(BasicClient):
    ...
    def on_emotion_detected(self, emotion_tag: str):
        """Overrides the hook in BasicClient to actually send data to Arduino"""
        logger.info(f"🤖 [CHECKPOINT] Sending gesture to Arduino: {emotion_tag}")
        # The list of hardware-supported tags
        valid_tags = ['greeting', 'wave', 'point', 'confused', 'shrug', 'angry', 'sad', 'sleep', 'default', 'pose', 'idle']
        
        # Clean up the tag to be lowercase
        clean_tag = emotion_tag.strip().lower()

        # Check if the AI hallucinated a fake tag
        if clean_tag not in valid_tags:
            logger.warning(f"⚠️ Invalid tag '{clean_tag}' generated by AI. Defaulting to default.")
            clean_tag = 'default'
            
        logger.info(f"🤖 [CHECKPOINT] Sending gesture to Arduino: {clean_tag}")
        
        # Send the validated lowercase tag to the Arduino module
        if self.arduino_module:
            self.arduino_module.send_command(clean_tag)

    def _register_custom_event_handlers(self):
        """Registers all client-specific WebSocket event handlers."""
        if self.server_connection and self.server_connection.sio:
            self.server_connection.sio.on('chat_response', self.on_chat_response)
            self.server_connection.sio.on('execute_command', self.on_execute_command)
    ...
    def on_chat_response(self, data):
        """Handle raw chat response routing and extract emotions for Arduino"""
        response_text = data.get('response')
        if not response_text: return

        # 1. Look for the emotion tag like [WAVE]
        match = re.search(r'\[(.*?)\]', response_text)
        if match:
            emotion_tag = match.group(1)
            # Trigger the Arduino!
            self.on_emotion_detected(emotion_tag) 
            
        # 2. Clean the text so EdgeTTS doesn't speak the tag out loud
        clean_text = re.sub(r'\[.*?\]', '', response_text).strip()

        # 3. Route to output modules
        if 'console_output' in self.output_modules:
            # Send raw text to console so you can still visually see the tags
    ...
    def on_execute_command(self, data):
        """Handle direct delegated commands"""
        ...
        if response:
            ...
            if response_text:
                ...
                logger.info(f"🗣️ Speaking response: '{clean_text[:100]}...'")
                if 'edge_tts_output' in self.output_modules:
                    self.output_modules['edge_tts_output'].process_output({'text': clean_text})

    def send_robot_emotion(self, emotion: str) -> bool:
        """Looks up an emotion tag and sends the corresponding command to the Arduino module."""
        command_to_send = self.emotion_map.get(emotion.upper(), "default")
        
        if hasattr(self, 'arduino_module') and self.arduino_module and self.arduino_module.is_connected():
            return self.arduino_module.send_command(command_to_send)
            
        logger.warning(f"⚠️ [CHECKPOINT] Cannot send emotion '{emotion}' - Arduino not connected")
        return False
    ...
    def setup_all_modules(self):
        ...
        if self.config.get('features', {}).get('arduino_integration', True):
            ...
            if self.register_output_module(self.arduino_module):
                ...
                logger.error("   ⚠️ Arduino output module failed to register")
