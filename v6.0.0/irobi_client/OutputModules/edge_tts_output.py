# OutputModules/edge_tts_output.py - Google TTS output module
import subprocess
import re
import threading
import queue
...
class EdgeTTSOutputModule(OutputModule):
    """
    Google TTS (gTTS) output with runtime voice config support.
    ...
    def start(self) -> bool:
        if not self.enabled:
            ...
            self.tts_thread = threading.Thread(
                target=self._tts_worker, daemon=True, name="irobi-tts"
            )
    ...
    def speak_with_callback(self, text: str, callback=None) -> bool:
        """Queue text for TTS; fire callback() after playback — used for demo step ACKs."""
        if not self.enabled:
            if callback:
                callback()
            return False
    ...
    def update_voice_config(self, voice_config: dict):
        ...
        Keys: language (str), gender ('female'|'male'), rate (str e.g. '+10%')
        """
